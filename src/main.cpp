...
const string strMessageMagic = "ChanceCoin Signed Message:\n";
...
{
    // ChanceCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // ChanceCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // ChanceCoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // ChanceCoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // ChanceCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // ChanceCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // ChanceCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5ac0d8ac8b02df41b3f987277cfc92845132a9ead0fa2b0b5a90af0eff6d747d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Chance is a word void of sense; nothing can exist without a cause.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1389815310;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 799064325;
        ...
        assert(block.hashMerkleRoot == uint256("0xb97130d03ec19b277a5992d2de6f5d98f57ac1582947e9477fa7d90384f695be"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // ChanceCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChanceCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChanceCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChanceCoinMiner(CWallet *pwallet)
{
    printf("ChanceCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("chancecoin-miner");
    ...
    try { loop {
        ...
        printf("Running ChanceCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ChanceCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChanceCoinMiner, pwallet));
}
